...
def insert_track(self, track):       
    ...
    sql = "INSERT INTO " + table + "(slice, day, cam, part, subpart, track_id, track_class, time, geom) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,ST_GeomFromEWKT(%s))"
    ...
    def run(self):
        ...
        with open(self.args['track_file_path'], 'r') as csv_file:
                ...
                for row in csv_reader:
                    ...
                    pointM = 'SRID=5555;POINTM(%s %s %s)' % (str(x_utm), str(y_utm), int(subpartstarttime + (image_id / framerate)))
                       
                    insert_track(self, (slicee, day, cam, part, subpart, track_id, track_class, timestamp.strftime('%Y-%m-%d %H:%M:%S'), pointM))
